...
?>
    ...
    <div class="row">
        ...
        <div class="card">
            ...
            <form class="row g-3" method="post" action="<?php echo site_url('dashboard/update_funcionario');?>">

                <input type="text" class="form-control" id="id_usuario_autenticacao" name="id_usuario_autenticacao" style="display: none">
                    ...
                    <div class="col-sm-4">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="papel" id="papel_label">
                                <!-- Campo para preenchimento em JS -->
                            </label>
                        </div>
                    </div>
...
// ========================================= -- PREENCHER CAMPOS -- =========================================
    ...
    if ( <?php echo (isset($dados_usuario)); ?> ) {
        dados_usuario = <?php echo (json_encode($dados_usuario)); ?>;
        
        document.getElementById('id_usuario_autenticacao').value = dados_usuario['id_usuario_autenticacao'];
        document.getElementById('nome').value = dados_usuario['nome'];
        document.getElementById('sobrenome').value = dados_usuario['sobrenome'];
        document.getElementById('email').value = dados_usuario['email'];
        document.getElementById('sexo').value = dados_usuario['genero'];
        document.getElementById('cpf').value = dados_usuario['cpf'];
        document.getElementById('celular').value = dados_usuario['celular'];
        ...
        if (dados_usuario['papel'] == 'Administrador') {
            campo_papel.checked = true;
            papel_label.innerHTML = "<?php echo lang('administrador');?>";
        } 
        
        if (dados_usuario['papel'] == 'Atendente') {
            campo_papel.checked = false;
        ...
        if (dados_usuario['status'] == true) {
            campo_status.checked = true;
            status_label.innerHTML = '<?php echo lang('ativo');?>';
        } else {
            campo_status.checked = false;
...
// ========================================= -- ENDEREÇO -- =========================================
    ...
    // Preenche campo do estado
    endereco = JSON.parse(dados_usuario['endereco']);
    ...
    if (estado.value != null) {
        ...
        // Atribui cidade vinda do banco
        cidades.value = endereco['city'];
    }
